...
const ProfilePage = () => {
  ...
  async function updateUserDetails(eve) {
    ...
    try {
      ...
      if (selectedFile) {
        formData.append("photo", selectedFile); // MUST match upload.single("photo")
      }
  ...
  // console.log('User ', `http://localhost:5000${user?.photoUrl}`)
  return (
            ...
            <form onSubmit={updateUserDetails}
              ...
              <div className='bg-base-300 p-[2%] rounded-lg flex flex-col justify-between w-full'>
                <div className='h-[45vh] w-full flex justify-center items-center'>
                  {
                  userForm?.photoUrl ? (
                    <img src={`http://localhost:5000${user?.photoUrl}`} alt={`${user.firstName} Photo`} className='w-full h-full' />
                  ) : (
